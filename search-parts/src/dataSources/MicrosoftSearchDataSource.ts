...
import { IMicrosoftPeopleResponse } from "../models/search/IMicrosoftPeopleResponse";
...
export class MicrosoftSearchDataSource extends BaseDataSource<IMicrosoftSearchDataSourceProperties> {
    ...
    private _avaliableQueryTypes: IComboBoxOption[] = [
        {
            key: QueryType.People,
            text: "People",
            data: ['birthday', 'companyName', 'department', 'displayName', 'givenName', 'jobTitle', 'mail', 'mailNickname', 'mobilePhone', 'officeLocation', 'preferredLanguage', 'surname', 'userPrincipalName']
        },
        {
            key: QueryType.Search,
            text: "Search Query",
            data: ['name', 'webUrl', 'filetype', 'createdBy', 'createdDateTime', 'lastModifiedDateTime', 'parentReference', 'size', 'description', 'file', 'folder', 'subject', 'bodyPreview', 'replyTo', 'from', 'sender', 'start', 'end', 'displayName', 'givenName', 'surname', 'userPrincipalName', 'phones', 'department']
        }
    ];
    ...
    */
    ...
    public async getData(dataContext: IDataContext): Promise<IDataSourceData> {
        ...
        if (this._properties.queryType !== null) {
            if (this._properties.queryType === QueryType.Search) {
                // Search query
                if (this._properties.entityTypes.length > 0) {
                    const searchQuery = await this.buildMicrosoftSearchQuery(dataContext);
                    results = await this.searchQuery(searchQuery);
                } else {
                    // If no entity is selected, manually set the results to prevent
                    // having the previous search results items count displayed.
                    this._itemsCount = 0;
                }
            } else if (this._properties.queryType === QueryType.People) {
                // People query
                const [ peopleQuery, itemsCount, pageNumber ] = await this.buildMicrosoftPeopleQuery(dataContext);
                results = await this.searchPeople(peopleQuery, itemsCount, pageNumber);
            }
        }
        return results;
    }
    ...
    public getPropertyPaneGroupsConfiguration(): IPropertyPaneGroup[] {
        ...
        const QueryTypesDisplayValue = this._avaliableQueryTypes.find(option => option.key === this.properties.queryType)?.text;

        let queryType: IPropertyPaneField<any>[] = [
            new PropertyPaneAsyncCombo('dataSourceProperties.queryTypes', {
                availableOptions: this._avaliableQueryTypes,
                allowMultiSelect: false,
                allowFreeform: false,
                description: "",
                label: commonStrings.DataSources.MicrosoftSearch.QueryTypesField,
                placeholder: "",
                searchAsYouType: false,
                defaultSelectedKey: this.properties.queryType,
                onPropertyChange: this.onCustomPropertyUpdate.bind(this),
                textDisplayValue: QueryTypesDisplayValue
            })
        ];

        let groupFields: IPropertyPaneField<any>[] = this.properties.queryType === QueryType.Search ? this.getSearchPropertyGroupsConfiguration() : this.getPeoplePropertyGroupsConfiguration();

        return [
            {
                groupName: commonStrings.DataSources.MicrosoftSearch.SourceConfigurationGroupName,
                groupFields: queryType.concat(...groupFields)
            }
        ];
    }

    private getSearchPropertyGroupsConfiguration(): IPropertyPaneField<any>[] {
        ...
        let commonFields: IPropertyPaneField<any>[] = [
            ...
            new PropertyPaneAsyncCombo('dataSourceProperties.searchFields', {
    ...
    private getPeoplePropertyGroupsConfiguration(): IPropertyPaneField<any>[] {
        let sortPropertiesFields: IPropertyPaneField<any>[] = [];
        let commonFields: IPropertyPaneField<any>[] = [
            new PropertyPaneAsyncCombo('dataSourceProperties.peopleFields', {
                availableOptions: this._availableFields,
                allowMultiSelect: true,
                allowFreeform: true,
                description: commonStrings.DataSources.MicrosoftSearch.SelectedFieldsPropertiesFieldDescription,
    ...
    public onCustomPropertyUpdate(propertyPath: string, newValue: any, changeCallback?: (targetProperty?: string, newValue?: any) => void): void {
        ...
        if (propertyPath.localeCompare('dataSourceProperties.queryTypes') === 0) {
            this.properties.queryType = (cloneDeep(newValue) as IComboBoxOption).key as QueryType;
        ...
        if (propertyPath.localeCompare('dataSourceProperties.searchFields') === 0 || propertyPath.localeCompare('dataSourceProperties.peopleFields') === 0) {
        ...
        return [
            {
                ...
                slotField: 'resource.displayName'
            },
            {
    ...
    private initProperties(): void {
        ...
        const CommonFields = {
            [QueryType.Search] : ["name", "webUrl", "filetype", "createdBy", "createdDateTime", "lastModifiedDateTime", "parentReference", "size", "description", "file", "folder", "subject", "bodyPreview", "replyTo", "from", "sender", "start", "end", "displayName", "givenName", "surname", "userPrincipalName", "phones", "department"],
            [QueryType.People] : ["displayName", "givenName", "surname", "birthday", "jobTitle", "companyName", "department", "officeLocation", "profession", "userPrincipalName", "phones"]
        };

        this.properties.queryType = this.properties.queryType !== undefined ? this.properties.queryType : QueryType.Search;

        this.properties.entityTypes = this.properties.entityTypes !== undefined ? this.properties.entityTypes : [EntityType.DriveItem];
        
        this.properties.fields = this.properties.fields !== undefined ? this.properties.fields : CommonFields[this.properties.queryType];
        ...
        if (this.properties.queryType === QueryType.Search) {
            if (this.properties.useBetaEndpoint) {
                this._microsoftSearchUrl = `${this.context?.pageContext?.legacyPageContext?.msGraphEndpointUrl}/beta/` + this.properties.queryType;
            } else {
                this._microsoftSearchUrl = `${this.context?.pageContext?.legacyPageContext?.msGraphEndpointUrl}/v1.0/` + this.properties.queryType;
            }
        } else {
            this._microsoftSearchUrl = `${this.context?.pageContext?.legacyPageContext?.msGraphEndpointUrl}/v1.0/` + this.properties.queryType;
        }
    ...
    private resetProperties(): void {
        ...
        this.properties.useBetaEndpoint = false;
        if (this.properties.queryType === QueryType.Search) {
            this.properties.entityTypes = [EntityType.DriveItem];
            this._microsoftSearchUrl = `${this.context?.pageContext?.legacyPageContext?.msGraphEndpointUrl}/v1.0/` + this.properties.queryType;
            this.properties.queryAlterationOptions = { enableModification: false, enableSuggestion: false };
            this.properties.queryTemplate = "{searchTerms}";
            this.properties.useBetaEndpoint = false;
            this.properties.enableResultTypes = false;
            this.properties.trimDuplicates = false;
            this.properties.contentSourceConnectionIds = [];

            this.properties.sortList = this.properties.sortProperties;
            this.properties.collapseProperties = [];

        } else {
            this._microsoftSearchUrl = `${this.context?.pageContext?.legacyPageContext?.msGraphEndpointUrl}/v1.0/` + this.properties.queryType;
        }
        
        this.properties.fields = CommonFields[this.properties.queryType];
        this._availableFields = this.properties.fields.map(field => {return { key: field, text: field } as IComboBoxOption;}) as IComboBoxOption[];

        this.properties.sortProperties = [];
    ...
    private async buildMicrosoftPeopleQuery(dataContext: IDataContext): Promise<[ string, number, number]> {
        let peopleQuery: string = this._microsoftSearchUrl + '?';
        ...
        // Selected properties
        let selectedProperties = this.properties.fields.filter(a => a); // Fix to remove null values

        let filterParams = `$filter=givenName ne null&`;
        let queryParams = `$orderby=surname`;

        if (queryText !== undefined && queryText !== "") {
            peopleQuery = peopleQuery + `$search=${queryText}&` + filterParams + queryParams;
        } else {
            peopleQuery = peopleQuery + filterParams + queryParams;
        }


       

        return [peopleQuery, dataContext.itemsCountPerPage, dataContext.pageNumber];
    }
     ...
     */
    private async searchQuery(searchQuery: IMicrosoftSearchQuery): Promise<IMicrosoftSearchDataSourceData> {
    ...
    private async searchPeople(peopleQuery: string, numItems: number, pageNumber: number): Promise<IMicrosoftSearchDataSourceData> {

        let query = peopleQuery+`&$top=${150}`;
        ...
        const msGraphClientFactory = this.serviceScope.consume<MSGraphClientFactory>(MSGraphClientFactory.serviceKey);
        const msGraphClient = await msGraphClientFactory.getClient('3');
        const jsonResponse: IMicrosoftPeopleResponse = await msGraphClient.api(query).get();
        itemsCount = await this.getPeopleTotalCount(peopleQuery);
        
        // Paging
        let from = 0;
        if (pageNumber > 1) {
            from = (pageNumber - 1) * numItems;
        }
        ...
        if (jsonResponse.value && Array.isArray(jsonResponse.value)) {
            ...
            // Map results
            const hits = jsonResponse.value.map(item => {

                interface hitProps {
                    "hitId": string,
                    "summary": string,
                    "resource": {[value:string]:string}
                }

                let hit : hitProps = {} as hitProps;
                ...
                Object.keys(item).forEach(field => {
                    hit["resource"][field] = item[field];
                });

                return hit;
            });

            response.items = response.items.concat(hits).splice(from, from+numItems);
    ...
    private async getPeopleTotalCount(peopleQuery: string): Promise<number> {
        let totalItemCount = 0;
        const msGraphClientFactory = this.serviceScope.consume<MSGraphClientFactory>(MSGraphClientFactory.serviceKey);
        const msGraphClient = await msGraphClientFactory.getClient('3');
        let jsonResponse: IMicrosoftPeopleResponse = await msGraphClient.api(peopleQuery + `&$top=${150}`).get();
        while(jsonResponse) {
            totalItemCount += jsonResponse.value.length;
            if(jsonResponse["@odata.nextLink"]) {
                jsonResponse = await msGraphClient.api(jsonResponse["@odata.nextLink"]).get();
            } else {
                break;
            }
        }
